	...
	Node.prototype.paint = function(context,node) {	// override Node.paint
		...
		function printLabel(context,node) {	// print Label
			...
			if ((node.stat==STAT.sel)||(node.stat==STAT.fix)) {
							...
							if (node.name.match('azul') == null){
			                	node.name = "azul-" + node.name;
			        		}
	...
	Edge.prototype.paint = function(context,edge) {	// override Edge.paint
		...
		if (edge.initv == Nodes[edge.termv].prev) {
			if ((istat==STAT.fix)&&(tstat==STAT.adj)) {
				...
				context.strokeStyle = 'blue';
				if (edge.name.match('fast') == null){
                	edge.name = "fast-" + edge.name;
        		}
			}
			context.lineWidth = 3;
		}
...
function buscar_node(nome) {
	var achou;
	for (var i = 0; i < Nodes.length; i++) {
		if (nome == Nodes[i].name) {
			return Nodes[i];
			achou = true;
		}
	}
...
function buscar_edge(nome) {
	var array_edges = [];
	var achou;
	for (var i = 0; i < Edges.length; i++) {
		if (Edges[i].name.search(nome) > 0) {
			array_edges.push(Edges[i]);
			achou = true;
		}
	}
	if (achou != true) {
